  ...
  function handle($match, $state, $pos, Doku_Handler $handler) {
   ...
   function render($mode, Doku_Renderer $renderer, $data) {
        if($mode == 'xhtml'){           
         ...
         //$renderer->doc .='<span class="clickerdir  tocselb hx1_5" id="tocsel_rootns"  title="' .  $rootopen . '" onclick="tocsel_updatetoc(\':*\');">'. $nsroot.'</span><br />';
         $renderer->doc .=  '<DIV><FORM><input type="hidden" value="' . $select. '" id="selectoc_btn"  name="selectoc_btn" /> <INPUT  type="hidden" title="wiki:id" id="selectoc_id" name="selectoc_id" value="'.$wikid .'"></FORM></DIV>';
